Public Class EDITarget850Class
    Inherits ClassBase
    ...
    Public Sub New(ByVal _dsOrderHeader As DataSet, ByVal _dsOrderLines As DataSet, ByVal _dsCustomerXref As DataSet)
        ...
        'Terms
        ' Look up Terms
        If Not (IsDBNull(_dsOrderHeader.Tables(0).Rows(0).Item("TERMS_DISC_PERC")) And _
                IsDBNull(_dsOrderHeader.Tables(0).Rows(0).Item("TERMS_DISC_DUEDAYS"))) Then

            Dim disc As Decimal = _dsOrderHeader.Tables(0).Rows(0).Item("TERMS_DISC_PERC")
            Dim days As String = _dsOrderHeader.Tables(0).Rows(0).Item("TERMS_DISC_DUEDAYS")

            Dim _paymentTerms As New PaymentTermsClass
            Dim myTermCode As String = _paymentTerms.GetTermCodebyValues(days, disc)

            _termsCode = myTermCode

        Else ' Use Default Term Code
        ...
        'Ordered By
        _OrderedBy = _dsOrderHeader.Tables(0).Rows(0).Item("PO_DATE")

        'FOB
        _FOB = _dsOrderHeader.Tables(0).Rows(0).Item("FOB_LOC_DESC")
        ...
        For Each myRow In _dsOrderLines.Tables("OrderLines").Rows
            ...
            If partNum <> "" Then
                ...
                newRow("BothNotes") = "CARTON COUNT: " & cartonCount
            Else
                newRow("PARTEXT") = ""
                newRow("PRTNUM") = ""
                newRow("BOTHDESC") = ""
        ...
        'Comment 1
        _Comment1 = "VENDOR ID:" & _dsOrderHeader.Tables(0).Rows(0).Item("VENDOR")
        _Comment1 &= " DEPT NO:" & _dsOrderHeader.Tables(0).Rows(0).Item("DEPT")
        ...
        'Ext Notes
        _ExtNotes = "SHIP NOT BEFORE: " & _dsOrderHeader.Tables(0).Rows(0).Item("DATE_1") & vbCrLf
        _ExtNotes &= "SHIP NO LATER: " & _dsOrderHeader.Tables(0).Rows(0).Item("DATE_2") & vbCrLf
        '_ExtNotes &= vbCrLf
        '_ExtNotes &= "ENTERED BY " & SalesOrderClass.
