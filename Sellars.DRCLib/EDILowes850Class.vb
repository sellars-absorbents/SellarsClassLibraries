Public Class EDILowes850Class
    ...
    Public Sub New(ByVal _dsOrderHeader As DataSet, ByVal _dsOrderLines As DataSet, ByVal _dsCustomerXref As DataSet)
        ...
        'Ordered By
        If _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("PO_TYPE_CODE") = "NS" Then
            _OrderedBy = _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("ADDR_LOC_CODE")
            _OrderedBy &= " NEW STORE"
        Else
            _OrderedBy = _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("ADDR_LOC_CODE")
        End If

        'FOB
        If _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("FOB") = "CC" Then
            _FOB = "ORIGIN"
        ElseIf _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("FOB") = "PP" Then
            _FOB = "DESTINATION"
        Else
            _FOB = "UNIDENTIFIED"
        End If
        ...
        'Customer
        _customer = New CustomerClass(_dsCustomerXref.Tables(0).Rows(0).Item("CUST_ID"), DataSource.Max)
        ...
        For Each myRow In _dsOrderLines.Tables("OrderLines").Rows
            ...
            newRow("CURDUE") = _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("DATE_2")
            newRow("CUSDUE") = _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("DATE_2")
            ...
            If partNum <> "" Then
                ...
                'Determine number of pallets.  Need at least 1 pallet per part
                palletCount += Math.Ceiling(Math.Round(myRow("QTY_ORD") * myPart.SLSCNV / _partChar.CasesPerPalletLTL, 2))
            Else
                newRow("PARTEXT") = ""
                newRow("PRTNUM") = ""
                newRow("BOTHDESC") = ""
            ...
            newRow("REQSHP") = Format(_dsOrderHeader.Tables("OrderHeader").Rows(0).Item("DATE_2"), "MM/dd/yy")
            newRow("ShortComment") = "Line Notes"
            newRow("Closed") = False

            newRow("Taxable") = _customer.Taxable
            newRow("TaxCode1") = _customer.TaxCode1
            newRow("TaxCode2") = _customer.TaxCode2
            newRow("TaxCode3") = _customer.TaxCode3
        ...
        If _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("FOB") = "CC" Then
            _ExtNotes = _ExtNotes.Replace("[LINE1]", "* SHIP VIA " & _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("CARRIER_ROUTING") & " COLLECT")
        ElseIf _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("FOB") = "PP" Then
            _ExtNotes = _ExtNotes.Replace("[LINE1]", "* SHIP FREE FREIGHT")
        End If
        _ExtNotes = _ExtNotes.Replace("[LINE2]", "* REQUESTED SHIP DATE: " & _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("DATE_2"))
        _ExtNotes = _ExtNotes.Replace("[LINE3]", "* REQUESTED DELIVERY DATE: " & _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("DATE_1"))
        If _dsOrderHeader.Tables("OrderHeader").Rows(0).Item("PO_TYPE_CODE") = "NS" Then
            _ExtNotes = _ExtNotes.Replace("[LINE4]", "")
        Else
            _ExtNotes = _ExtNotes.Replace("[LINE4]", "* TOTAL PALLETS: " & palletCount.ToString)
        End If
